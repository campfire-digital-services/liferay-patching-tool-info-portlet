...
public class PatchingToolCommandRunner {
	...
	private List<String> patchingToolOptions = new ArrayList<>();
	...
	public void runCommand() throws Exception {
		...
		try {
			...
			// NOTE: ProcessBuilder#environent is initialised with
			// System.getenv()
			// @see
			// http://docs.oracle.com/javase/7/docs/api/java/lang/ProcessBuilder.html#environment%28%29
			...
			// eg. boolean finished = iostat.waitFor(100,
			// TimeUnit.MILLISECONDS);

			int processExitValue = process.waitFor();

			List<String> processOutputLines = IOUtils.readLines(process.getInputStream(), Charset.defaultCharset());

			List<String> processErrorLines = IOUtils.readLines(process.getErrorStream(), Charset.defaultCharset());
			...
			if (LOG.isDebugEnabled()) {
				...
				LOG.debug(
						"patching tool returned " + this.patchingToolResults.getOutputLines().size() + " output lines");
			...
			// NOTE: Command shell may return lines in the error stream that are
			// warning messages, not errors.
			// Hence, we cannot rely upon content in the error stream as a valid
			// error.
